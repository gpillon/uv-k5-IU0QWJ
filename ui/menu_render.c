/* Menu rendering - loaded as EEPROM overlay on demand.
 * Split from ui/menu.c to keep the large display function
 * out of flash (overlay system loads it into RAM). */
    ...
    if (UI_MENU_GetCurrentMenuId() == MENU_SLIST1 || UI_MENU_GetCurrentMenuId() == MENU_SLIST2 || UI_MENU_GetCurrentMenuId() == MENU_SLIST3)
    {
        ...
        if (gSubMenuSelection < 0 || !gEeprom.SCAN_LIST_ENABLED[i]) {
            ...
            for (uint8_t pri = 1; pri <= 2; pri++) {
                uint16_t channel = (pri == 1) ? gEeprom.SCANLIST_PRIORITY_CH1[i] : gEeprom.SCANLIST_PRIORITY_CH2[i];
