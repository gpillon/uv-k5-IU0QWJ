...
MEM_FORMAT = """
// ===== Section 1: MR + VFO channel records (999 MR + 14 VFO @ 0x2000) =====
#seekto 0x2000;
struct {
  ul32 freq;
  ul32 offset;
...
// ===== Section 2: Channel attributes (1006 bytes, padded to 1008) =====
#seekto 0x5F50;
struct {
u8 is_scanlistx:3,
compander:2,
...
// ===== Section 3: Channel names (999 x 16 @ 0x6340) =====
#seekto 0x6340;
struct {
char name[16];
} channelname[999];

// ===== Section 4: Settings (original low EEPROM addresses) =====
// FM presets 48 bytes @ 0x0E40 (20 ul16 + 8 pad)
#seekto 0x0E40;
ul16 fmfreq[20];
u8 __fm_preset_pad[8];

// General settings 1 @ 0x0E70
#seekto 0x0E70;
u8 call_channel;
u8 squelch;
u8 max_talk_time;
...
// FM radio runtime config @ 0x0E88 (8 bytes; firmware uses 4 packed + pad)
#seekto 0x0E88;
u8 fm_config_raw[8];
...
// Password @ 0x0E98 (8 bytes in EEPROM; CHIRP uses ul32 + pad)
#seekto 0x0E98;
ul32 password;
u8 __password_pad[4];
...
// Alarm / Roger / battery type @ 0x0EA8
#seekto 0x0EA8;
u8 alarm_mode;
u8 roger_beep;
u8 rp_ste;
...
// AES key @ 0x0F30 (opaque; not exposed in UI)
#seekto 0x0F30;
u8 aes_key[16];
...
// VFO indices (uint16_t) @ 0x0F50 (16 bytes)
#seekto 0x0F50;
...
// ===== Section 5: DTMF contacts @ 0x1C00 =====
#seekto 0x1C00;
struct {
char name[8];
char number[3];
...
// ===== Section 6: Calibration @ 0x1E00-0x1FEF (+ build block below) =====
struct {
    struct {
        #seekto 0x1E00;
        u8 openRssiThr[10];
...
// ===== Section 7: Build options / F4HWN @ 0x1FF0 =====
#seekto 0x1FF0;
struct {
...
// ===== Code overlays (opaque binary, written during firmware setup) =====
// Spectrum overlay (up to 8KB)
#seekto 0xA200;
u8 ovly_spectrum[8192];

// FM radio overlay (up to 4KB)
#seekto 0xC200;
u8 ovly_fm[4096];

// Aircopy overlay (up to 4KB)
#seekto 0xD200;
u8 ovly_aircopy[4096];

// Menu rendering overlay (up to 6KB)
#seekto 0xE200;
u8 ovly_menu[6144];
...
# Total EEPROM image (see eeprom_map.h EEPROM_TOTAL_SIZE)
MEM_SIZE = 0xA1B0
# Extended image size including overlay region
MEM_SIZE_WITH_OVERLAYS = 0xFA00
# Full programming image
PROG_SIZE = MEM_SIZE_WITH_OVERLAYS
MEM_BLOCK = 0x80  # largest block of memory that we can reliably write
# Factory calibration block (never written unless upload_calibration is True)
CAL_START = 0x1E00
# F4HWN tail fields after 2-byte BUILD_OPTIONS at 0x1FF0 (0x1FF0 + 2)
F4HWN_START = 0x1FF2

# Code overlay EEPROM addresses (must match eeprom_map.h)
OVLY_SPECTRUM_ADDR = 0xA200
OVLY_FM_ADDR       = 0xC200
OVLY_AIRCOPY_ADDR  = 0xD200
OVLY_MENU_ADDR     = 0xE200
OVLY_END_ADDR      = 0xFA00

# Normal upload: disjoint ranges (skip 0x1E00-0x1FEF factory cal; see do_upload).
UPLOAD_RANGES_NORMAL = (
    (0x0E40, 0x0F60),
    (0x1C00, 0x1D00),
    (0x2000, MEM_SIZE),
    (OVLY_SPECTRUM_ADDR, OVLY_END_ADDR),
)
# Second pass: F4HWN bytes after BUILD_OPTIONS (matches legacy two-pass behaviour)
UPLOAD_RANGE_F4HWN_TAIL = (F4HWN_START, 0x2000)
...
def do_download(radio):
    ...
    if f:
        ...
        LOG.info("Connected - firmware: %s", f)
    else:
        raise errors.RadioError("Failed to initialize radio")

    download_phases = [
        (0x0000, MEM_SIZE,      "Settings + Channels"),
        (OVLY_SPECTRUM_ADDR, OVLY_END_ADDR, "Code overlays"),
    ]

    total_bytes = sum(e - s for s, e, _ in download_phases)
    status.max = total_bytes

    LOG.info("Download plan: %d bytes total", total_bytes)

    for range_start, range_end, label in download_phases:
        if len(eeprom) < range_start:
            eeprom += b"\xff" * (range_start - len(eeprom))
        phase_size = range_end - range_start
        addr = range_start
        while addr < range_end:
            chunk = min(MEM_BLOCK, range_end - addr)
            data = _readmem(serport, addr, chunk)
            done = addr - range_start + chunk
            status.cur += chunk
            status.msg = "Reading %s  0x%04X-0x%04X  (%d / %d)" % (
                label, range_start, range_end, done, phase_size)
            radio.status_fn(status)
            if data and len(data) == chunk:
                ...
                raise errors.RadioError(
                    "Download incomplete at 0x%04X (%s)" % (addr, label))

    status.msg = "Download complete!"
    radio.status_fn(status)
    LOG.info("Download complete - %d bytes read", total_bytes)

    return memmap.MemoryMapBytes(eeprom)


def _upload_range(serport, mmap, start, end, status, status_fn, phase_label):
    """Write one contiguous [start, end) range, updating status.msg with
    a live byte counter so the user can see exactly what is happening."""
    total = end - start
    addr = start
    while addr < end:
        chunk = min(MEM_BLOCK, end - addr)
        dat = mmap[addr:addr + chunk]
        if len(dat) != chunk:
            raise errors.RadioError(
                "%s: incomplete read at 0x%04X" % (phase_label, addr))
        _writemem(serport, dat, addr)
        done = addr - start + chunk
        status.cur += chunk
        status.msg = "%s  0x%04X-0x%04X  (%d / %d bytes)" % (
            phase_label, start, end, done, total)
        status_fn(status)
        addr += chunk


# Upload phase descriptors: (start, end, label)
# Built dynamically in do_upload() depending on calibration mode.
...
def do_upload(radio):
    ...
    if f:
        ...
        raise errors.RadioError("Failed to connect to radio")

    # --- Build phase list ---
    if radio.upload_calibration:
        phases = [
            (CAL_START, F4HWN_START, "Calibration"),
            (F4HWN_START, 0x2000,    "F4HWN settings"),
        ]
    else:
        phases = [
            (0x0E40, 0x0F60,              "Settings & FM presets"),
            (0x1C00, 0x1D00,              "DTMF contacts"),
            (F4HWN_START, 0x2000,         "F4HWN settings"),
            (0x2000, MEM_SIZE,            "Channels (999 MR + VFO + attrs + names)"),
            (OVLY_SPECTRUM_ADDR, OVLY_SPECTRUM_ADDR + 0x2000,
                                          "Overlay: Spectrum analyzer"),
            (OVLY_FM_ADDR, OVLY_FM_ADDR + 0x1000,
                                          "Overlay: FM radio"),
            (OVLY_AIRCOPY_ADDR, OVLY_AIRCOPY_ADDR + 0x1000,
                                          "Overlay: Aircopy"),
            (OVLY_MENU_ADDR, OVLY_MENU_ADDR + 0x1800,
                                          "Overlay: Menu"),
        ]

    total_bytes = sum(e - s for s, e, _ in phases)
    status.max = total_bytes

    mmap = radio.get_mmap()

    LOG.info("Upload plan: %d phases, %d bytes total", len(phases), total_bytes)
    for s, e, label in phases:
        LOG.info("  0x%04X - 0x%04X  %5d bytes  %s", s, e, e - s, label)

    # --- Execute phases ---
    for start, end, label in phases:
        _upload_range(serport, mmap, start, end, status, radio.status_fn, label)

    status.msg = "Upload complete!"
    radio.status_fn(status)
    LOG.info("Upload complete - %d bytes written", total_bytes)
...
# bugfix calibration : put in comment next line: upload_calibration = False
    ...
    def get_features(self):
        ...
        # This radio supports memories 1-999, 1000-1013 are the VFO memories
    ...
    def _inject_overlays(self):
        """Inject overlay binaries into the EEPROM image before upload.
        Looks for .ovly_*.bin files next to this driver script."""
        import os
        script_dir = os.path.dirname(os.path.abspath(__file__))
        overlays = [
            ("f4hwn.ovly_spectrum.bin", OVLY_SPECTRUM_ADDR, "Spectrum"),
            ("f4hwn.ovly_fm.bin",       OVLY_FM_ADDR,       "FM radio"),
            ("f4hwn.ovly_aircopy.bin",  OVLY_AIRCOPY_ADDR,  "Aircopy"),
            ("f4hwn.ovly_menu.bin",     OVLY_MENU_ADDR,     "Menu"),
        ]
        mmap = self.get_mmap()
        for fname, addr, label in overlays:
            fpath = os.path.join(script_dir, fname)
            if os.path.exists(fpath):
                with open(fpath, "rb") as f:
                    data = f.read()
                LOG.info("Overlay %-10s: %5d bytes -> EEPROM 0x%04X  [%s]",
                         label, len(data), addr, fname)
                for i, byte in enumerate(data):
                    mmap[addr + i] = byte
            else:
                LOG.warning("Overlay %-10s: FILE NOT FOUND [%s] "
                            "- feature will be disabled on radio",
                            label, fpath)
    ...
    def get_memory(self, number):
        ...
        if ch_num > 998:
            mem.name = self._get_vfo_channel_names()[ch_num-999]
    ...
    def set_settings(self, settings):
        ...
        for element in settings:
            ...
            if elname == "VFO_A_chn":
                ...
                if _mem.ScreenChannel_A < 999:
                    _mem.MrChannel_A = _mem.ScreenChannel_A
                elif _mem.ScreenChannel_A < 1006:
            ...
            # VFO_B
                ...
                if _mem.ScreenChannel_B < 999:
                    _mem.MrChannel_B = _mem.ScreenChannel_B
                elif _mem.ScreenChannel_B < 1006:
...
#
        ...
        append_label(help_group,"(Mxxx) = Memory number M1-M999", "On the left side of the VFO" )
...
#       unlock.append(en_scrambler_setting)
    ...
    def set_memory(self, memory):
        ...
        att_num = number if number < 999 else 999 + int((number - 999) / 2)
...
#        _mem_attr.is_free = 0
        ...
        # channels >999 are the 14 VFO channels and don't have names
        if number < 999:
            _mem_chname = self._memobj.channelname[number]
